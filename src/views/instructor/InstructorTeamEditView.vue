<template>
    ...
    <div class="centered-text">
        To add students to a team, please select a team, then select all the students you wish to add to that team and
    ...
    <div v-if="hasLoaded" class="page">
        ...
        <div class="teams">
            ...
            <div class="team" v-for="team in teams" :key="team.id">
                {{ team.name }}
                <input type="checkbox" :value="team.id" :checked="team.id === selectedTeamId"
    ...
    <!-- Instructor Section -->
    <div class="assistant-instructor">
        <h2>Assistant Instructors</h2>
        <div v-for="instructor in assistantInstructors" :key="instructor.id">
...
export default {
    ...
    methods: {
        ...
        clearSelection() {
            ...
            this.getStudents()
            this.getTeams()
            this.getAssistantInstructors()
        },
        getAssistantInstructors() {
            this.isLoading = true;
            ...
            apiClient.get(`http://localhost:80/api/v1/assistantInstructor/getInstructor/${storeSection.selectedSectionId}`,
                { headers: { 'Authorization': `Bearer ${auth}` } }
            )
                .then(response => {
                    console.log(response.data); // Log the response data
                    this.assistantInstructors = response.data.data;
                    this.hasLoaded = true;
                })
                .catch(error => {
                    console.log(error);
    ...
    created() {
        ...
        this.getTeams()
        this.getAssistantInstructors()
        console.log("Assistant Instructors in created hook:", this.assistantInstructors); // Add this log
    },
}
