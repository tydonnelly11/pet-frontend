<template>
   ...
   <h3>
      To add students to a team, please select a team, then select all the
      students you wish to add to that team and click the save button.
   </h3>
...
export default {
   ...
   methods: {
      getStudents() {
         ...
         axios
            .get(
               `http://localhost:80/api/v1/section/getAllStudents/${storeSection.selectedSectionId}`,
               { headers: { Authorization: `Bearer ${auth}` } }
            )
            .then((response) => {
               this.students = []
      ...
      getTeams() {
         ...
         axios
            .get(
               `http://localhost:80/api/v1/section/getAllTeams/${storeSection.selectedSectionId}`,
               { headers: { Authorization: `Bearer ${auth}` } }
            )
            .then((response) => {
      ...
      createTeams() {
         ...
         axios
            .post(
               `http://localhost:80/api/v1/team/save`,
               {
                  id: null,
                  name: this.teamName,
                  sectionId: storeSection.selectedSectionId,
