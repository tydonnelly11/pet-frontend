  ...
  methods: {
    async fetchRubricAndReports() {
      try {
        ...
        const rubricResponse = await apiClient.get(`http://localhost:80/api/v1/section/getRubric/${storeSection.selectedSectionId}`, {
      ...
      try {
        ...
        if (response.data.flag && response.data.code === 200 && response.data.data.length > 0) {
          ...
          for (const student of studentReport) {
            ...
            const score = student.averageScore ? `${student.averageScore}/${this.totalScore}` : 'NO PEER EVALUATION SUBMITTED';
            tempReports.push({
        ...
        this.error = 'An error occurred while fetching the evaluation reports.';
      }
