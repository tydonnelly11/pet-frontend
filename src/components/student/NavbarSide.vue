<template>
   <div :class="['sidebar-container', { open: isSidebarOpen }]">
      <div class="logo-details" @click="toggleSidebar">
         <div class="icon" :class="isSidebarOpen ? 'close' : ''"></div>
      </div>
      <ul
         class="nav nav-pills flex-column justify-content"
         v-if="isSidebarOpen"
      >
         <li class="nav-item">
            <router-link @click="toggleSidebar" to="/studenthome/team"
               >Team</router-link
            >
         </li>
         <li class="nav-item">
         ...
         <li class="nav-item">
            <router-link @click="toggleSidebar" to="/studenthome/studentwar"
               >War</router-link
            >
         </li>
         <li class="nav-item">
...
.logo-details .icon::before,
.logo-details .icon::after {
   content: '';
   position: absolute;
   left: 50%;
   width: 30px;
...
.logo-details .icon.close {
   background-color: transparent;
}

.logo-details .icon.close::before {
   transform: translateX(-50%) rotate(45deg);
   top: 0;
}

.logo-details .icon.close::after {
   transform: translateX(-50%) rotate(-45deg);
   bottom: 0;
}
